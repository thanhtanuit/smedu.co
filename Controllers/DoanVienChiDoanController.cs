...
namespace Bidc_Union_Management.Controllers
{
    ...
    public class DoanVienChiDoanController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            List<DoanVien> doanviens = new List<DoanVien>();
            if (Roles.IsUserInRole("Viewer"))
            {
               doanviens = db.DoanViens.Where(a => a.UserName != "admin"
                                                    &a.Branch==dv.Branch
                                                    &a.Status==2)
            ...
            foreach (DoanVien item in doanviens)
            {
                ...
                dsdoanvien.Add(new DoanvienModels()
                {
                    Id = item.Id,
        ...
        {
            ViewBag.doanvien = "active";
            DoanVien Objectdv = db.DoanViens.SingleOrDefault(a => a.UserName == User.Identity.Name);
            List<ChidoanModel> dschidoan = new List<ChidoanModel>();
            List<ChiDoan> listcd = db.ChiDoans.Where(a => a.Id == Objectdv.ChiDoanId).OrderBy(a => a.Name).ToList();
            foreach (ChiDoan item in listcd)
            {
                ...
                {
                    List<DoanvienModels> dsdoanvien = new List<DoanvienModels>();
                    List<DoanVien> listdv = db.DoanViens.Where(a => a.PhanDoanId == pd.Id 
                                            & a.ChiDoanId == Objectdv.ChiDoanId
                                            &a.UserName!="admin"
                                            &a.Status==2)
                                            .ToList();
                ...
                dschidoan.Add(new ChidoanModel()
                {
                    Id = item.Id,
                    Name = item.Name,
        ...
        public ActionResult AuthoriseConfirmed(Guid id = new Guid())
        {
            ViewBag.doanvien = "active";
            DoanVien doanvien = db.DoanViens.Find(id);
            doanvien.Status = 2;
            db.SaveChanges();
            return Redirect("/DoanVienChiDoan/index");
        }
        ...
        public ActionResult Create()
        {
            ...
            List<PhanDoan> dsphandoan = db.PhanDoans
                                    .Where(a=>a.ChiDoanId==dv.ChiDoanId)
                                    .OrderBy(a => a.Name)
                                    .ToList();
        ...
        public ActionResult Create(DoanVien doanvien)
        {
            ...
            if (ModelState.IsValid)
            {
                if (doanvien.UserName != null & doanvien.Password != null)
                {
                    ...
                    Roles.AddUserToRole(doanvien.UserName, "Viewer");
        ...
        public ActionResult DeleteConfirmed(Guid id)
        {
            ViewBag.doanvien = "active";
            DoanVien doanvien = db.DoanViens.Find(id);
            if (doanvien.Status == 1 || doanvien.Status == 3)
            {
                db.DoanViens.Remove(doanvien);
                db.SaveChanges();
            }
            else if (doanvien.Status == 2)
            {
                doanvien.Status = 3;
                db.SaveChanges();
            }
